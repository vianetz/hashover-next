...
final class SettingsHandler extends AbstractHandler
{
    /**
     * Generates an array of various settings information
     */
    private function ui_array(Setup $setup, Locale $locale)
    {
        // Theme names
        $themes = array();
     ...
     */
    private function create_select($hashover, $name, array $setting)
    {
    ...
    private function create_paragraph($hashover, $name, array $setting)
    {
        ...
        switch ($setting['type']) {
            ...
            {
                $label->createAttribute('class', 'block');
                $element = $this->create_select($hashover, $name, $setting);
    ...
    public function __invoke(): void
    {
        ...
        if (isset ($_POST['save'])) {
            ...
            $settings_file = $this->hashover->setup->getAbsolutePath('config/settings.json');

            // Read JSON settings file
            $json = $this->dataFiles->readJSON($settings_file);
            ...
            if ($this->hashover->login->verifyAdmin() === true) {
                ...
                $saved = $this->dataFiles->saveJSON($settings_file, $settings);
        ...
        foreach ($ui as $locale => $settings) {
            ...
            if (!empty ($this->hashover->locale->text[$locale])) {
                $locale = $this->hashover->locale->text[$locale];
            }
            ...
            foreach ($settings as $name => $setting) {
                $items->appendChild($this->create_paragraph($this->hashover, $name, $setting));
            }
        ...
        // Template data
        $template = array(
            'title' => $this->hashover->locale->text['settings'],
            'sub-title' => $this->hashover->locale->text['settings-sub'],
            'settings' => $div->asHTML("\t"),
            'save-button' => $this->hashover->locale->text['save']
        );

        echo $this->parse_templates('admin', 'settings.html', $template, $this->hashover);
    }
}